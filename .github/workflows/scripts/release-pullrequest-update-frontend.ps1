...
Write-Host "Starting Updating Frontend Services for PR from $source_environment to $target_environment"

$environments = Get-Content -Path ".github/workflows/config/environments.json" | ConvertFrom-Json
$components = Get-Content -Path ".github/workflows/config/components.json" | ConvertFrom-Json | Where-Object { $_.componentType -eq "frontend" -and $_.isOwnRepo -eq "true"}
...
foreach ($component in $components) {
    ...
    $sourceComponentFolder = "frontend/$sourceEnvironmentFolderName/$sourceRegionShortName/$componentFolderName"
    $targetComponentFolder = "frontend/$targetEnvironmentFolderName/$targetRegionShortName/$componentFolderName"
    ...
    if ($sourceArtifact -ne $existingTargetArtifact) {
        ...
        Write-Host "Frontend Component Name: $($component.componentName), New Version: $sourceVersion, Target artifact file: $targetArtifactFile"
